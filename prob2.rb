...
if response.code == 200
  ...
  sc_data = crypto_data.sort_by { |crypto| -crypto['market_cap'] }

  puts "Top 5 Crypto by Market Capitalization"
  sc_data.take(5).each_with_index do |crypto, index|
    puts "#{index + 1}. #{crypto['name']}: $#{crypto['current_price']} (Market Cap: $#{crypto['market_cap']})"
  end
else
  puts 'Failed to access crypto data'
end
